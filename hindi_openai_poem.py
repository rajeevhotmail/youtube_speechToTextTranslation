...
def generate_poem(username, message):
    ...
    prompt = f"Write a 4-line poem for a live chat user. The user's name is {username}, and their message is: '{message}'. the poem should contain word troll."
    try:
...
def generate_hindi_poem(username, message):
    ...
    The user's name is {username}, and their message is: '{message}'. the poem should contain the word troll"""
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4-turbo",  # Replace with "gpt-4" if desired
            messages=[
...
def detect_and_warn_abuse(live_chat_id, messages, start_time):
    ...
    for message in messages:
        ...
        if normalized_author_name in target_users:
            ...
            poem = generate_hindi_poem(raw_author_name, user_message)
            poem = ":robot: " + poem
            print(f"Generated poem for {raw_author_name}:\n{poem}")
            post_message(live_chat_id, poem)

            print(f"Poem sent to {raw_author_name}")
            poem_english = generate_poem(raw_author_name, user_message)
            post_message(live_chat_id, poem_english)
            print(f"Generated poem for {raw_author_name}:\n{poem_english}")
